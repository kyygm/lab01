...
class UnrolledLinkedList:
    ...
    def set(self, index, value):
        ...
        if 0 <= index < new_list.total_size:
            i = 0
            node = new_list.head
            while i <= index:
                for ele in node.elements[0:node.node_size]:
    ...
    def add(self, element):
        new_list = copy.deepcopy(self)
        new_list._insert(new_list.tail, new_list.tail.node_size, element)
        return new_list
    ...
    def from_list(self, lst):
        ...
        if len(lst) > 0:
            for e in lst:
                new_list._insert(new_list.tail, new_list.tail.node_size, e)
        return new_list
    ...
    def remove(self, value):
        ...
        while node is not None:
            ...
            for ele in node.elements[0:node.node_size]:
                if ele == value:
                    new_list._remove_from_node(node, index)
                index += 1
            node = node.next
        return new_list
    ...
    def mconcat(self, lst):
        ...
        if lst is not None:
            for i in range(0, node.node_size):
                new_list._insert(new_list.tail, new_list.tail.node_size, node.elements[i])
            node = node.next
        return new_list
    ...
    def filter(self, f):
        new_list = copy.deepcopy(self)
        event_list = list(filter(f, new_list.to_list()))
